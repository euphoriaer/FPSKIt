        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (pb)
            {
                ...
                if (pb.isController && !pb.isBot)
                {
                    ...
                    pb.main.hud.DisplayLeavingBattlefield(timeUntilDeath);
                }
            }
        }
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            if (pb)
            {
                for (int i = 0; i < playersOutOfMap.Count; i++)
                {
                    if (playersOutOfMap[i].pb == pb)
                    {
                        if (playersOutOfMap[i].pb.isController && !playersOutOfMap[i].pb.isBot)
                        {
                            //Update hud in order to hide it
                            playersOutOfMap[i].pb.main.hud.DisplayLeavingBattlefield(-1);
                        }
        ...
        private void Update()
        {
            ...
            for (int i = playersOutOfMap.Count - 1; i >= 0; i--)
            {
                if (playersOutOfMap[i].pb)
                {
                    ...
                    if (playersOutOfMap[i].timeLeft <= 0)
                    {
                        if (playersOutOfMap[i].pb.photonView.IsMine)
                        {
                    ...
                    {
                        //Update hud
                        playersOutOfMap[i].pb.main.hud.DisplayLeavingBattlefield(playersOutOfMap[i].timeLeft);
                    }
                }
                else
                {
