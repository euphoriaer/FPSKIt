...
namespace MarsFPSKit
{
    ...
    public class Kit_MinimapDefault : Kit_MinimapBase
    {
        ...
        public override void LocalPlayerSwitchedTeams(Kit_IngameMain main)
        {
        ...
        public override void PlayerShoots(Kit_PlayerBehaviour pb, bool silenced)
        {
            if (!silenced)
            {
                if (activePlayers.ContainsKey(pb))
                {
                    ...
                    if (main.currentPvPGameModeBehaviour.AreWeEnemies(main, pb.isBot, pb.isBot ? pb.botId : pb.photonView.Owner.ActorNumber))
                    {
        ...
        public override void PlayerSpawned(Kit_PlayerBehaviour pb)
        {
            if (!activePlayers.ContainsKey(pb))
            {
                ...
                if (main.currentPvPGameModeBehaviour && main.currentPvPGameModeBehaviour.AreWeEnemies(main, pb.isBot, pb.isBot ? pb.botId : pb.photonView.Owner.ActorNumber))
                {
        ...
        public override void Setup(Kit_IngameMain nMain)
        {
            //Assign main
            main = nMain;
            //Disable camera
