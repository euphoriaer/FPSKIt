    ...
    public class Kit_PlayerDominationBotControl : Kit_PlayerBotControlBase
    {
        ...
        public override void WriteToPlayerInput(Kit_PlayerBehaviour pb)
        {
            ...
            if (Time.time - 0.1f > kbcrd.lastRoutine)
            {
                ...
                {
                    if (kbcrd.nma.remainingDistance <= 0.5f)
                    {
                ...
                {
                    if (kbcrd.nma.remainingDistance > 10f && !pb.input.lmb)
                    {
                ...
                if (Time.time - 5f > kbcrd.lastForceNewWaypoint || !kbcrd.playerToEngage && (kbcrd.nma.remainingDistance < 3f || !kbcrd.nma.hasPath))
                {
        ...
        {
            DominationRuntimeData drd = pb.main.currentGameModeRuntimeData as DominationRuntimeData;
            ...
            for (int i = 0; i < drd.flags.Length; i++)
            {
                ...
                {
                    //Check distance
                    if (Vector3.Distance(pb.transform.position, drd.flags[i].flagObject.transform.position) < nearestFlagDistance)
                    {
                        nearestFlagDistance = Vector3.Distance(pb.transform.position, drd.flags[i].flagObject.transform.position);
