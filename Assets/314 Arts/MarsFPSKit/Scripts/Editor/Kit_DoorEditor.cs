...
public class Kit_DoorEditor : Editor
{
    ...
    void Awake()
    {
        ...
        PhotonView pv = door.GetComponent<PhotonView>();

        if (!pv)
        {
            pv = door.gameObject.AddComponent<PhotonView>();
        }

        pv.ObservedComponents = new List<Component>();

        if (!pv.ObservedComponents.Contains(door))
        {
            pv.ObservedComponents.Add(door);
        }
